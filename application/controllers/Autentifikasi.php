...
{ 
    ...
    { 
        ...
        { $data['judul'] = 'Login'; $data['user'] = ''; 
            ...
            $data); $this->load->view('autentifikasi/login'); 
    ...
    { 
        ...
        { 
            ...
            { 
                ...
                { $data = [ 'email' => $user['email'], 'role_id' => $user['role_id'] ]; 
                    ...
                    else
                    {
                        ...
                        { 
                            ...
                            ('pesan', '<div class="alert alert-info alert-message" role="alert">
                ...
                { $this->session->set_flashdata
                    ('pesan', '<div class="alert alert-danger alert-message" role="alert">Password salah!!</div>'); 
                    redirect('autentifikasi'); 
                } 
            } 
            else 
            {$this->session->set_flashdata
                ('pesan', '<div class="alert alert-danger alert-message" role="alert">User belum diaktifasi!</div>'); 
                redirect('autentifikasi'); 
            } 
        } 
        else 
        { 
            $this->session->set_flashdata
            ('pesan', '<div class="alert alert-danger alert-message" role="alert">Email tidak terdaftar!</div>'); 
            redirect('autentifikasi'); 
        } 
    }
    ...
    { 
        ...
        //membuat rule untuk inputan password agar tidak boleh kosong, tidak ada spasi, tidak boleh kurang dari dari 3 digit, 
        //dan password harus sama dengan repeat password dengan membuat pesan error dengan 
        ...
        //jika disubmit kemudian validasi form diatas tidak berjalan, maka akan tetap berada ditampilan registrasi. 
        //tapi jika disubmit kemudian validasi form diatas berjalan, maka data yang diinput akan disimpan ke dalam tabel user
        if ($this->form_validation->run() == false) { $data['judul'] = 'Registrasi Member'; 
        ...
        { 
            ...
            $this->session->set_flashdata(
                ...
                '<div class="alert alert-success alert-message" role="alert">
                    Selamat!! akun member anda sudah dibuat. Silahkan Aktivasi Akun anda</div>'); 
            redirect('autentifikasi');
        }
    } 
    ...
    // Method yang berfunsi ntuk mengontrol blok.php
    public function blok() 
    { $this->load->view('autentifikasi/blok'); } 

    // Method yang berfunsi ntuk mengontrol gagal.php
