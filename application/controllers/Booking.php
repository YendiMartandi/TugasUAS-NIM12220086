...
{ 
    ...
    { 
        ...
        $this->session->userdata('email')])->row_array(); foreach ($user as $a) 
        { 
    ...
    { 
        ...
        if ($temp > 0) { $this->session->set_flashdata('pesan', '<div class="alert alert-danger alert-message" role="alert">Buku ini Sudah anda booking </div>'); 
    ...
    public function bookingSelesai($where) 
    {
        ...
        $tglsekarang = date('Y-m-d'); $isibooking = 
            [ 
    ...
    { 
        ...
        $this->load->library('dompdf_gen'); $this->load->view('booking/bukti-pdf', $data); $paper_size = 'A4'; 
